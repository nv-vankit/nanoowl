...
class ClipPredictor(torch.nn.Module):
    ...
    def __init__(self,
            model_name: str = "hf-hub:apple/DFN5B-CLIP-ViT-H-14-378",
        ...
        super().__init__()
        self.device = device
        self.clip_model, self.preprocess = create_model_from_pretrained(model_name)
    ...
    def encode_text(self, text: List[str]) -> ClipEncodeTextOutput:
        text_tokens = self.tokenizer(text, return_tensors="pt", padding=True).to(self.device)
    ...
    def predict(self, 
            ...
            text: List[str], 
            text_encodings: Optional[ClipEncodeTextOutput] = None,
        ...
        rois = torch.tensor([[0, 0, image_tensor.shape[2], image_tensor.shape[3]]], dtype=image_tensor.dtype, device=image_tensor.device)
